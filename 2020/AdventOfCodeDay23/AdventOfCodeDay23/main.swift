...
//    }
    for _ in 1...3 {
        ...
        if pickUpIndex >= input.count {
            pickUpArray.append(input.remove(at: 0))
        } else {
            //print(input)
    ...
    // set dest
    for x in 1...3 {
        if !pickUpArray.contains(currentVal-x) {
            dest = currentVal-x
            hasLowerDest = true
            break
        }
...
//print(input, currentVal)
let x = input.firstIndex(of: 1)
print(input[x!+1]*input[x!+2], Date().timeIntervalSince(start))
