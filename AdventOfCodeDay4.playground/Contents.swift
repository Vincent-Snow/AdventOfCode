...
var passport = input.components(separatedBy: "\n\n")
var counter = 0
var numValidPassports = 0
var passportDict: [String: String] = [:]
var testString = "160cm"
let testSubString = testString.prefix(while: { "0"..."9" ~= $0 })
print(Int(testSubString)!)

for i in 0...passport.count-1 {
    let individualPassportArray = passport[i].components(separatedBy: "\n").flatMap({$0.components(separatedBy: " ")})
    print(individualPassportArray)
    for i in 0...individualPassportArray.count-1 {
        let property = individualPassportArray[i].components(separatedBy: ":")
        passportDict[property[0]] = property[1]
    }
    //print(passportDict)
    for field in passportDict  {
        print("key is \(field.key)")
        switch field.key {
        case "byr":
            if (Int(field.value) ?? 0 >= 1920 && Int(field.value) ?? 0 <= 2002) {
                counter+=1
            }
        case "iyr":
            if (Int(field.value) ?? 0 >= 2010 && Int(field.value) ?? 0 <= 2020) {
                counter+=1
            }
        case "eyr":
            if (Int(field.value) ?? 0 >= 2020 && Int(field.value) ?? 0 <= 2030) {
                counter+=1
            }
        case "hgt":
            let hgt = Int(field.value.prefix(while: { "0"..."9" ~= $0 })) ?? 0
        ...
        case "hcl":
            hairIf: if field.value[0] == "#" && field.value.count == 7 {
                for x in 1...field.value.count-1 {
                    switch field.value[x] {
