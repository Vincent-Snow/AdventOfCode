...
var counterPt2 = 0
var numValidPasswordsPt2 = 0
var inputArray = input.components(separatedBy: "\n") /// parsing the input
for i in 0...inputArray.count-1 {
    let splitArray = inputArray[i].split(separator: ":", maxSplits: 1, omittingEmptySubsequences: true) /// getting individual password/policy
    let policy = splitArray[0] /// set policy
    var password = splitArray[1] /// set password
    password.removeFirst()

    let betweenArray = policy.split(separator: " ")[0].split(separator: "-")   /// create array for acceptable between values for counter
    let intBetweenArray = betweenArray.map { Int($0) ?? 0} /// convert array to Int
    
    if password[intBetweenArray[0]-1] == policy[policy.count-1] {
        counterPt2+=1
    }
    if password[intBetweenArray[1]-1] == policy[policy.count-1] {
        counterPt2+=1
    }
    if counterPt2 == 1 {
...
for i in 0...inputArray.count-1 {
    let splitArray = inputArray[i].split(separator: ":", maxSplits: 1, omittingEmptySubsequences: true) /// getting individual password/policy
    let policy = splitArray[0] /// set policy
    let password = splitArray[1] /// set password
    
    for i in password {
        if i == policy[policy.count-1] {            /// set counter equal to the number of occurences of the policy letter
            counter+=1
            print(counter)
        }
    }
    let betweenArray = policy.split(separator: " ")[0].split(separator: "-")   /// create array for acceptable between values for counter
    if counter >= Int(betweenArray[0])! && counter <= Int(betweenArray[1])! {
        print("hooray!")
        print(policy)
        print(password)
        numValidPasswords+=1
